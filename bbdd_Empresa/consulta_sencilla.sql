...
--3
select nomem as 'Nombre', extel as 'Extension Telefonica' from Empleados where numde = 110 order by extel asc;

--4
select comis, nomem, salar from Empleados where numhi = 3 order by comis desc;

--5
select comis, nomem, salar from Empleados where numhi = 3 and comis is NOT NULL order by comis asc;

--6
select salar, nomem from Empleados where salar > 1200 and salar < 1500 and numhi = 0;
